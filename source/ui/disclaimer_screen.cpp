...
void DisclaimerScreen::renderTop(C3D_RenderTarget *target) {
	...
	drawCenteredRichText(25.0f, 0.5f, 0.75f, 0.75f, ScreenManager::colorWhite(), "Disclaimer", 400.0f);

	float lineW = 80.0f;
	C2D_DrawRectSolid(centerX - lineW / 2, 48.0f, 0.5f, lineW, 2.0f, ScreenManager::colorAccent());

	std::vector<std::string> lines = MessageUtils::wrapText(DISCLAIMER_TEXT, 360.0f, 0.45f);
	float y = 65.0f;
	for (const auto &line : lines) {
		drawCenteredRichText(y, 0.5f, 0.45f, 0.45f, ScreenManager::colorWhite(), line, 400.0f);
		y += 16.0f;
	}
}
...
void DisclaimerScreen::renderBottom(C3D_RenderTarget *target) {
	...
	drawCenteredText(centerY - 40.0f, 0.5f, 0.5f, 0.5f, ScreenManager::colorWhite(), "Do you agree to these terms?",
	                 320.0f);

	drawCenteredText(centerY + 10.0f, 0.5f, 0.5f, 0.5f, ScreenManager::colorSuccess(), "A: Agree & Continue", 320.0f);

	drawCenteredText(centerY + 40.0f, 0.5f, 0.5f, 0.5f, ScreenManager::colorError(), "B: Reject & Exit", 320.0f);
}
