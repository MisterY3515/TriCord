...
UpdateScreen::UpdateScreen(const std::string& downloadUrl, const std::string& assetName)
	...
	statusText = Utils::String::format(i18n.get("updater.downloading"), assetName);
...
void UpdateScreen::render(bool isTopScreen) {
	if (isTopScreen) {
		...
		if (totalBytes > 0 && !updateComplete) {
			...
			std::string progressStr = Utils::String::format(i18n.get("updater.progress"), 
				Utils::String::format("%zu KB", currentBytes / 1024), 
				Utils::String::format("%zu KB", totalBytes / 1024));
