...
void ThemeManagerScreen::refreshThemes() {
  ...
  std::vector<std::string> filenames =
      Config::getInstance().getAvailableThemes();
  ...
  for (const auto &fname : filenames) {
    ...
    std::string path =
        std::string(CONFIG_DIR_PATH) + "/themes/" + fname + ".json";
...
void ThemeManagerScreen::update() {
  ...
  if (deleteState == DeleteState::CONFIRMING) {
    if (kDown & KEY_A) {
      if (themeToDeleteIndex > 0 &&
          themeToDeleteIndex - 1 < (int)themes.size()) {
  ...
  if (kDown & KEY_A) {
    ...
    if (config.getSelectedThemeName() == themeFile &&
        config.isCustomThemeEnabled()) {
  ...
  if (kDown & KEY_Y) {
    ...
    if (!item.updateUrl.empty() && !isUpdating) {
      ...
      downloadThread = std::thread(&ThemeManagerScreen::downloadTheme, this,
                                   item.updateUrl, item.filename);
    }
  }
...
void ThemeManagerScreen::downloadTheme(const std::string &url,
  ...
  if (resp.success) {
    std::string path =
        std::string(CONFIG_DIR_PATH) + "/themes/" + filename + ".json";
    ...
    updateStatus = Core::I18n::format(TR("theme.status.failed_code"),
...
void ThemeManagerScreen::renderTop(C3D_RenderTarget *target) {
  ...
  C2D_DrawRectSolid(0, 0, 0.9f, topScreenWidth, headerH,
                    ScreenManager::colorHeaderGlass());
  C2D_DrawRectSolid(0, headerH - 1.0f, 0.91f, topScreenWidth, 1.0f,
                    ScreenManager::colorHeaderBorder());

  std::string titleText = TR("theme.manager.title");
  drawCenteredText(4.0f, 0.95f, 0.52f, 0.52f, theme.text, titleText,
                   topScreenWidth);
  ...
  for (int i = (int)scrollOffset; i < totalItems; i++) {
    ...
    if (isSel) {
      drawRoundedRect(x, y + 4, 0.55f, 4, itemHeight - 8.0f, 2.0f,
                      theme.selection);
    }
    ...
    drawRoundedRect(toggleX, toggleY, 0.6f, toggleW, toggleH, toggleH / 2.0f,
                    tBg);
    float circSize = 14.0f;
    float circX =
        isActive ? (toggleX + toggleW - circSize - 2.0f) : (toggleX + 2.0f);
    drawCircle(circX + circSize / 2.0f, toggleY + circSize / 2.0f + 2.0f, 0.65f,
               circSize / 2.0f, theme.pure_white);
  ...
  if (deleteState == DeleteState::CONFIRMING) {
    ...
    drawCenteredText(cy + 12.0f, 0.92f, 0.5f, 0.5f, theme.text, title,
                     topScreenWidth);

    drawCenteredText(cy + 42.0f, 0.92f, 0.45f, 0.45f, theme.text_muted,
                     TR("theme.delete.desc"), topScreenWidth);

    std::string help =
        "\uE000: " + TR("common.delete") + "  \uE001: " + TR("common.cancel");
    drawCenteredText(cy + ch - 22.0f, 0.92f, 0.4f, 0.4f, theme.error, help,
                     topScreenWidth);
  }
}
...
void ThemeManagerScreen::renderBottom(C3D_RenderTarget *target) {
  ...
  if (!desc.empty()) {
    std::vector<std::string> lines =
        MessageUtils::wrapText(desc, bottomScreenWidth - 20, 0.45f);
  ...
  drawText(10.0f, infoY + 15.0f, 0.6f, 0.45f, 0.45f, theme.text_muted,
           TR("theme.tip.desc"));
  drawText(10.0f, infoY + 30.0f, 0.6f, 0.45f, 0.45f, theme.text,
           "sdmc:/3ds/TriCord/themes/");

  float keysY = 240.0f - 25.0f;

  std::string help = "\uE000: " + TR("theme.help.select") +
                     "  \uE001: " + TR("theme.help.back");
  if (!themes.empty()) {
