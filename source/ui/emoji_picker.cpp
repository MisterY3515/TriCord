...
void EmojiPicker::handleCategoryChange(int delta) {
  ...
  currentCategoryIndex =
      (currentCategoryIndex + categories.size() + delta) % categories.size();
  emojiIndex = 0;
  targetEmojiScrollY = 0.0f;
  emojiScrollY = 0.0f;
}
...
void EmojiPicker::update(u32 kDown, u32 kHeld, u32 kUp, circlePosition circle) {
          ...
          if (idx >= 0 && idx < (int)currentCat.emojiIndices.size()) {
            if (idx == pendingTapIndex) {
              if (onEmojiSelected) {
                ...
                onEmojiSelected(Utils::Utf8::hexToUtf8(
                    allCodepoints[currentCat.emojiIndices[idx]]));
              }
              pendingTapIndex = -1;
            } else {
              emojiIndex = idx;
  ...
  if (kDown & KEY_UP)    { up = true;    keyRepeatTimer = 0; }
  if (kDown & KEY_DOWN)  { down = true;  keyRepeatTimer = 0; }
  if (kDown & KEY_LEFT)  { left = true;  keyRepeatTimer = 0; }
  if (kDown & KEY_RIGHT) { right = true; keyRepeatTimer = 0; }

  if (kHeld & (KEY_UP | KEY_DOWN | KEY_LEFT | KEY_RIGHT)) {
    keyRepeatTimer++;
    if (keyRepeatTimer >= REPEAT_INITIAL_DELAY &&
        (keyRepeatTimer - REPEAT_INITIAL_DELAY) % REPEAT_INTERVAL == 0) {
      if (kHeld & KEY_UP)    up = true;
      if (kHeld & KEY_DOWN)  down = true;
      if (kHeld & KEY_LEFT)  left = true;
      if (kHeld & KEY_RIGHT) right = true;
    }
  } else {
    keyRepeatTimer = 0;
  }
  ...
  if (up || down || left || right) {
    ...
    if (selRow < firstVisibleRow || selRow > lastVisibleRow) {
      ...
      if (up)    emojiIndex -= emojisPerRow;
      if (down)  emojiIndex += emojisPerRow;
      if (left)  emojiIndex--;
      if (right) emojiIndex++;
  ...
  if (kDown & KEY_A && onEmojiSelected) {
    const auto &allCodepoints = emojiMgr.getCodepoints();
    onEmojiSelected(Utils::Utf8::hexToUtf8(
        allCodepoints[currentCat.emojiIndices[emojiIndex]]));
  }
...
void EmojiPicker::render(C3D_RenderTarget *target,
                         const Discord::Message *activeMsg) {
  auto &emojiMgr = EmojiManager::getInstance();
  const auto &categories = emojiMgr.getCategories();
  const auto &allCodepoints = emojiMgr.getCodepoints();
  ...
  drawRoundedRect(0, 0, 0.45f, 320, HEADER_H, 0.0f,
                  ScreenManager::colorBackgroundLight());

  const float tabW = 320.0f / 8.0f;
  const char *tabIcons[] = {
      "romfs:/discord-icons/reaction.png",
      "romfs:/discord-icons/nature.png",
      "romfs:/discord-icons/food.png",
      "romfs:/discord-icons/gamecontroller.png",
      "romfs:/discord-icons/bicycle.png",
      "romfs:/discord-icons/object.png",
      "romfs:/discord-icons/heart.png",
      "romfs:/discord-icons/flag.png",
  };
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (tex) {
      ...
      C2D_PlainImageTint(&tint,
                         (i == currentCategoryIndex) ? ScreenManager::colorWhite()
                                                     : ScreenManager::colorText(),
                         1.0f);
      C2D_DrawImageAt(img, i * tabW + (tabW - iconSize) / 2.0f, 9.0f, 0.47f,
                      &tint, iconSize / tex->width, iconSize / tex->height);
    }
    if (i == currentCategoryIndex)
      drawRoundedRect(i * tabW, HEADER_H - 2, 0.47f, tabW, 2, 0.0f,
  ...
  for (size_t i = startIndex; i < endIndex; i++) {
    ...
    if (isSelected)
      drawRoundedRect(x + 1, y + 1, 0.41f, CELL_SIZE - 2, CELL_SIZE - 2, 4.0f,
                      C2D_Color32(110, 110, 115, 255));

    if (isReacted) {
      float pad = isSelected ? 3.0f : 2.0f;
      drawRoundedRect(x + pad, y + pad, isSelected ? 0.415f : 0.405f,
                      CELL_SIZE - pad * 2, CELL_SIZE - pad * 2, 3.0f,
                      ScreenManager::colorSelection() & 0xFFFFFF88);
    }
    ...
    if (info.tex) {
      ...
      Tex3DS_SubTexture sub = {(u16)info.originalW, (u16)info.originalH,
                               0.0f, 1.0f,
      ...
      drawRoundedRect(x + 5, y + 5, 0.415f, CELL_SIZE - 10, CELL_SIZE - 10,
                      4.0f, C2D_Color32(50, 50, 55, 255));
    }
  }
